...
static int error_setenv(char **input, data_t *data, int *ac)
{
    if (!input)
        return (ERROR);
    for ( ; input[*ac] ; (*ac)++);
    if (*ac == 1) {
        my_env(data, marg2("env\0"));
        return (ERROR);
    }
    if (*ac > 3) {
...
int my_setenv(data_t *data, char **input)
{
    char **new = NULL;
    char *arg = NULL;
    int row = 0;
    ...
    if (error_setenv(input, data, &ac) == ERROR)
        return (SUCCESS);
    if (is_word_in_arr_env(input[1], data->envp) == FALSE) {
        for ( ; data->envp[row] ; row++);
        if ((new = malloc(sizeof(char **) * (row + 2))) == NULL)
            return (ERROR);
        new[row + 1] = NULL;
        if (malloc_col_se(new, data->envp, input[1]) == ERROR)
            return (ERROR);
        fill_new_se(new, data->envp, input[1]);
        marr_free(data->envp);
        data->envp = new;
    }
    arg = input[2] ? input[2] : NULL;
    str_add_arg_se(data->envp, input[1], arg);
    return (SUCCESS);
}
