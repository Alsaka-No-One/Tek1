...
int my_unsetenv(data_t *data, char **input)
{
    char **new = NULL;
    int row;
    int row_envp = -1;
    ...
    if (error_unsetenv(input) == ERROR)
        return (SUCCESS);
    if (malloc_row_use(&new, data->envp, input) == ERROR)
        return (ERROR);
    for (row = 0 ; (data->envp)[row] ; row++)
        if (malloc_col_use(&new[row], data->envp, input, &row_envp) == ERROR)
            return (ERROR);
    row_envp = -1;
    for (row = 0 ; (data->envp)[row] ; row++)
        fill_new_use(&new[row], data->envp, input, &row_envp);
    marr_free(data->envp);
    data->envp = new;
    return (SUCCESS);
}
