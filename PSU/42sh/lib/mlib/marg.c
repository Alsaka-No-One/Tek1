...
static int calc_nb_arg(char *str, char c)
{
    int row = 0;
    int nb_arg = 0;

    for ( ; str[row] ; row++)
        if (str[row] != c && str[row] != '\n' &&
            (!str[row + 1] || str[row + 1] == c ||
            str[row + 1] == '\n' || str[row + 1] == '\n'))
            nb_arg++;
    return (nb_arg);
}

static int calc_nb_char(char *str, int row, char c)
{
    int nb_char = 0;

    for ( ; str[row] && (str[row] == c || str[row] == '\n') ; row++);
    for ( ; str[row] ; row++) {
        nb_char++;
        if (str[row] != c && str[row] != '\n' &&
            (!str[row + 1] || str[row + 1] == c ||
...
static char *fill_arg(char *str, int turn, char c)
{
    int row = 0;
    int nb_char = 0;
    char *arg = NULL;
    ...
    for (int nb_arg = 0 ; str[row] && nb_arg < turn ; row++)
        if (str[row] != c && str[row] != '\n' &&
            ...
            nb_arg++;
    nb_char = calc_nb_char(str, row, c);
